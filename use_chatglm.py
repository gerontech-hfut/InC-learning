...
def ask_glm(prompt,output_dir,temperature=0,result_num=1):
  max_retries = 5
  retries = 0
  while retries < max_retries:
    try:
      ...
      for i in completion.choices:
        ...
        f_output.write(i.message.content+ '\n')
        f_output.close()
        count=count+1
      ...
      if retries < max_retries:
        print('Failed to access API. Retrying in 30 seconds...')
        time.sleep(30)  # 等待30秒
      else:

        print('Failed to access API after multiple retries.')
        break
  return 0
...
def main(output_path,dataset_name,ICL_mode='random',cluster_num=10,ICL_sample_num=10,test_number=999999,instruction="Summarize the following table in one sentence ||"):
  ...
  for one_input_idx in range(len(all_input)):
    ...
    result=ask_glm(full_prompt,output_dir)
